		...
		link: function(scope, ele, attr){
			ele.on('click', function(){
	        	...
	        	scope.gauth.signIn().then(function(googleUser){
	        		...
	        		$rootScope.$broadcast('event:social-sign-in-success', {token: access_token, name: profile.getName(), email: profile.getEmail(), uid: profile.getId(), provider: "google", imageUrl: profile.getImageUrl()});
	        	}, function(err){
	        		console.log(err);
	        	})
	        });
